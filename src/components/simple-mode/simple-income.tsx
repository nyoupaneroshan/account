...
export function SimpleIncome() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const narration = `Income: ${notes || category} (${category}) via ${paymentMethod}${vatInclusive ? ' (VAT Inclusive)' : ''} [BS: ${adToBs(date).formattedBs}]`
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              {parsedAmount > 0 && (
                <div className="p-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                  <span className="text-zinc-400 font-medium">अक्षरमा (In Words):</span>
                  <span className="font-semibold text-emerald-400">{numberToNepaliWords(parsedAmount)}</span>
                </div>
              )}
