...
export function PurchaseForm() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([partyData, productData]) => {
        ...
        // Extract warehouses from product stock levels
        const whSet = new Map<string, WarehouseData>()
        prodList.forEach((p: Product & { stockLevels?: { warehouse: { id: string; name: string; isDefault: boolean } }[] }) => {
          if (p.stockLevels) {
  ...
  const handleSubmit = async (status: 'draft' | 'received') => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(status === 'draft' ? 'Purchase bill saved as draft' : 'Purchase bill created')
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <Badge variant="secondary" className="text-sm font-mono">{billNumber}</Badge>
      </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="space-y-1.5">
              ...
              <Input value={billNumber} disabled className="bg-muted" />
            </div>
            <div className="space-y-1.5">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[110px]">Cost Price</TableHead>
